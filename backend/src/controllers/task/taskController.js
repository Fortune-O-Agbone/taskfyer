...
export const getTasks = asyncHandler(async (req, res) => {
    try {
        ...
        res.status(200).json({
            length: tasks.length,//get the length of the task
            tasks,//return the length of the task
        });
    } catch (error) {
...
export const updateTask = asyncHandler(async (req, res) => {
    try {
        ...
        const task = await TaskModel.findById(id);//not creating an instance but finding the task

        //when the taks isn't found
        if (!task) {
            res.status(404).json({ message: "Task not found!" });
        }
